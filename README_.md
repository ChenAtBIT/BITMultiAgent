...
- Multi Agent协作架构：遵循A2A协议实现Agent间通信，采用“意图识别 Orchestrator + 专业 Agent”协作模式，引入运维Agent、纪要Agent、知识库Agent等角色，突破单 Agent 在上下文容量与能力边界上的限制。

- ReAct执行框架：设计“思考 -> 工具调用 -> 观察 -> 再思考”的闭环执行范式，增强Agent对复杂任务的自主分解、推理和工具调用能力。

- 分层记忆管理器：将对话信息划分为即时工作上下文、会话级短期摘要和持久化长期核心记忆三层，分层控制Token开销，并支持跨轮次、跨会话的知识延续；在10组长上下文配置中，将平均Prompt长度从9424压至7236，平均压缩率达23.2%。

- Agent能力扩展机制：基于MCP协议构建工具调用框架，集成文件读写、服务器巡检、文档向量化与检索等20+个工具，可通过运行时动态库加载实现可插拔扩展。

- 智能工具选择 ：在领域工具白名单的基础上，设计检索增强生成（RAG）Top-K 工具选择机制；在 100 条工具性请求上，平均注入工具数从 6 降至 2.5，整体工具调用成功率提升 12% 。

技术栈：C++/gRPC，Multi Agent，ReAct，A2A协议，Function call，MCP协议，RAG，SQLite
...
│ │ + MCP Client││ 无工具直答  ││ + MCP Client││ + MCP Client││+ MCP Client││ │
│ └──────┬──────┘└──────┬─────┘└──────┬──────┘└──────┬──────┘└────┬─────┘│   │
└────────┼──────────────┼─────────────┼──────────────┼─────────────┼─────────┘
         │              │             │              │             │
         └──────────────┼─────────────┴──────────────┴─────────────┘
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         MCP Server                                   │   │
│  │            Tool Registry + Plugin Loader + Tool Dispatcher           │   │
│  └─────────────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────────┘
```
